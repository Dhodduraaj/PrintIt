...
vec3 background_color(vec2 uv) {
  ...
  return col * 0.3; // Reduced from 0.5 to 0.3 for lower brightness
}
...
vec3 getLineColor(float t, vec3 baseColor) {
  ...
  float m = uv.y - y;
  return (0.0175 / max(abs(m) + 0.01, 1e-3) + 0.01) * 0.7; // Reduced brightness by 30%
}
  ...
  if (enableTop) {
    for (int i = 0; i < topLineCount; ++i) {
      ...
      col += lineCol * wave(
        ...
        mouseUv,
        interactive
      ) * 0.07; // Reduced from 0.1 to 0.07
    }
  }

  fragColor = vec4(col * 0.8, 1.0); // Overall brightness reduction
}
