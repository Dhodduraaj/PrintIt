  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-purple-50 to-indigo-100 flex items-center justify-center">
        <div className="text-xl text-purple-900 font-semibold">Loading jobs...</div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-50 to-indigo-100 py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-8">
          <h1 className="text-4xl font-bold text-purple-900 mb-2">🏪 Vendor Dashboard</h1>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          <div className="bg-white rounded-xl shadow-lg p-6 text-center border-2 border-purple-200">
            <div className="text-4xl font-bold text-purple-600 mb-2">{jobs.filter((j) => j.status === 'waiting').length}</div>
          ...
          <div className="bg-white rounded-xl shadow-lg p-6 text-center border-2 border-blue-200">
            <div className="text-4xl font-bold text-blue-600 mb-2">{jobs.filter((j) => j.status === 'printing').length}</div>
          ...
          <div className="bg-white rounded-xl shadow-lg p-6 text-center border-2 border-yellow-200">
            <div className="text-4xl font-bold text-yellow-600 mb-2">{jobs.filter((j) => j.status === 'pending').length}</div>
            <div className="text-sm font-semibold text-gray-600">Payment Pending</div>
          </div>
          <div className="bg-white rounded-xl shadow-lg p-6 text-center border-2 border-green-200">
            <div className="text-4xl font-bold text-green-600 mb-2">{jobs.filter((j) => j.status === 'done').length}</div>
        ...
        <div className="space-y-4">
          {filteredJobs.length === 0 ? (
            ...
            filteredJobs.map((job) => (
              <div key={job._id} className="bg-white rounded-xl shadow-lg p-6 hover:shadow-xl transition-all">
                <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                  <div className="flex-1">
                    <div className="flex items-center gap-4 mb-4">
                      <h3 className="text-2xl font-bold text-purple-900">Token #{job.tokenNumber}</h3>
                    ...
                    <div className="grid md:grid-cols-2 gap-3 text-gray-700">
                      <p><strong className="text-purple-900">Student:</strong> {job.student?.name || 'N/A'}</p>
                      <p><strong className="text-purple-900">Document:</strong> {job.fileName}</p>
                      <p><strong className="text-purple-900">Pages:</strong> {job.pageCount} × {job.copies} copies</p>
                      <p><strong className="text-purple-900">Type:</strong> {job.printType === 'color' ? 'Color' : 'Black & White'}</p>
                      {job.upiReferenceId && (
                        <p className="col-span-2"><strong className="text-purple-900">UPI Ref:</strong> <code className="bg-gray-100 px-2 py-1 rounded">{job.upiReferenceId}</code></p>
                      )}
                    </div>
                  </div>
